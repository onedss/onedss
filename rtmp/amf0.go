...
var (
	ErrAmfInvalidType = errors.New("invalid amf0 type")
	ErrAmfTooShort    = errors.New("too short to unmarshal amf0 data")
	ErrAmfNotExist    = errors.New("not exist")
)
...
func (amf0) ReadArray(b []byte) (ObjectPairArray, int, error) {
	...
	if len(b)-index >= 3 && bytes.Equal(b[index:index+3], Amf0TypeMarkerObjectEndBytes) {
		...
		log.Println("amf ReadArray without suffix Amf0TypeMarkerObjectEndBytes.")
	}
