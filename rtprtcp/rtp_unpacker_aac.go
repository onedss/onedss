...
func (unpacker *RtpUnpackerAac) TryUnpackOne(list *RtpPacketList) (unpackedFlag bool, unpackedSeq uint16) {
	...
	//log.Printf("%d, %d, %s", len(pkt.Raw), pkt.Header.timestamp, hex.Dump(b))
	...
	if len(aus) == 1 {
		...
		for {
			...
			if p.Packet.Header.Timestamp != timestamp {
				log.Printf("fragments of the same access shall have the same timestamp. first=%d, curr=%d",
					timestamp, p.Packet.Header.Timestamp)
				return false, 0
			}
			...
			aus := parseAu(b)
			if len(aus) != 1 {
				log.Printf("shall be a single fragment. len(aus)=%d", len(aus))
				return false, 0
			}
			if aus[0].size != totalSize {
				log.Printf("fragments of the same access shall have the same size. first=%d, curr=%d",
					totalSize, aus[0].size)
				return false, 0
			}
			...
			if cacheSize < totalSize {
				...
				log.Printf("cache size bigger then total size. cacheSize=%d, totalSize=%d",
					cacheSize, totalSize)
				return false, 0
			}
		}
...
func parseAu(b []byte) (ret []au) {
	...
	if (nbAuHeaders > 1 && pau != uint32(len(b))) ||
		...
		log.Printf("rtp packet size invalid. nbAuHeaders=%d, pau=%d, len(b)=%d, auHeadersLength=%d", nbAuHeaders, pau, len(b), auHeadersLength)
	}
