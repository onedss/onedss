...
func (unpacker *RtpUnpackerAvcHevc) TryUnpackOne(list *RtpPacketList) (unpackedFlag bool, unpackedSeq uint16) {
	...
	case PositionTypeStapa:
		...
		for i := 0; i != len(buf); {
			if len(buf)-i < 2 {
				log.Println("invalid STAP-A packet.")
				return false, 0
			}
	...
	case PositionTypeFuaStart:
		...
		for {
			...
			if p.Packet.positionType == PositionTypeFuaMiddle {
				...
				// 不应该出现其他类型
				log.Printf("invalid position type. position=%d", p.Packet.positionType)
				return false, 0
			}
		}
	...
	case PositionTypeFuaEnd:
		// noop
	default:
		log.Printf("invalid position. pos=%d", first.Packet.positionType)
	}
...
func calcPositionIfNeededAvc(pkt *RtpPacket) {
	...
	if outerNaluType <= NaluTypeAvcSingleMax {
		...
		log.Printf("unknown nalu type. outerNaluType=%d", outerNaluType)
	}
...
func calcPositionIfNeededHevc(pkt *RtpPacket) {
	...
	default:
		...
		log.Printf("unknown nalu type. outerNaluType=%d(%d), header=%+v, len=%d",
